...
public class PatternMatching {
        ...
        try {
            ...
            while (i <= text.length() - 1){
                ...
                for (int j = pattern.length() - 1; j >= -1; j--){
                    char current = text.charAt(searchText);
                    if (j == -1){
                        counts.add(searchText + 1);
                        i = i + 1;
                        break;
                    }
                    if (pattern.charAt(j) == text.charAt(searchText)){
                        searchText--;
    ...
    public static Map<Character, Integer> buildLastTable(CharSequence pattern) {
        // WRITE YOUR CODE HERE (DO NOT MODIFY METHOD HEADER)!
        Map<Character, Integer> hashMapTable = new HashMap<>();
        for (int i = 0; i < pattern.length(); i++){
            if (hashMapTable.get(pattern.charAt(i)) == null){
                hashMapTable.put(pattern.charAt(i), i);
